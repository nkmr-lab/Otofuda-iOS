...
final class MenuVC: UIViewController {
    ...
    var selectedMusics: [MusicModel] = []

    var playMusics: [MusicModel] = []

    var cardLocations: [Int] = []

    var presetLists: [PresetListResponse] = []
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        FirebaseManager.shared.post(path: room.url() + "musicCounts/\(me.order)", value: haveMusics.count)
    ...
    @IBAction func tappedStartBtn(_: Any) {
        ...
        case .preset:
            ...
            AF.request(SELECT_MUSIC_API_URL, method: .get, parameters: ["id": preset.id, "count": CARD_MAX_COUNT]).response { response in
                ...
                do {
                    let musicList: MusicListResponse = try JSONDecoder().decode(MusicListResponse.self, from: data)
                    let songs: [MusicResponse] = musicList.musics
                    for song in songs {
                        let music = MusicModel(entity: song)
    ...
    func loadApiPresets() {
        ...
        AF.request(PRESET_LIST_API_URL, method: .get, parameters: ["count": CARD_MAX_COUNT]).response { response in
            guard let data = response.data else { return }
            do {
                let presetsResponse: PresetApiResponse = try JSONDecoder().decode(PresetApiResponse.self, from: data)
