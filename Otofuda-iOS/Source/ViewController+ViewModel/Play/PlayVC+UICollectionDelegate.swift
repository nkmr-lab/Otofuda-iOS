...
extension PlayVC: UICollectionViewDelegate {
    func collectionView(_: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if tappedMusic.name == playingMusic.name {
            ...
            FirebaseManager.shared.post(path: room.url() + "answearUser/\(me.order)", value: ["time": Firebase.ServerValue.timestamp(), "userIndex": me.order])
        }
        // 不正解
        else {
        ...
        FirebaseManager.shared.post(path: room.url() + "tapped/\(me.order)", value: dict)
    }
}
